...
func init() {
	...
	utilruntime.Must(directoryv1alpha1.AddToScheme(scheme))
...
func main() {
	...
	if err = (&controllers.DirectoryServiceReconciler{
		Client: mgr.GetClient(),
		Log:    ctrl.Log.WithName("controllers").WithName("DirectoryService"),
	...
	if os.Getenv("ENABLE_WEBHOOKS") == "true" {
		...
		if err = (&directoryv1alpha1.DirectoryService{}).SetupWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "DirectoryService")
			os.Exit(1)
		}
	}
	if err = (&controllers.DirectoryBackupReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
